...
En una sesión nueva de `codex exec --ephemeral`, Codex reconoció los seis nombres de perfil registrados y cada uno respondió limited al faltar sus fuentes o etapas previas. El revisor también produjo BLOCK, REVISE y PASS en fixtures sintéticos. Se ajustó su contrato para mantener `blockers` y `unknowns` fuera de `payload` y para devolver arrays vacíos sin placeholders cuando corresponde. Los handoffs pasaron checks estructurales con schemas temporales por estado. El CLI no admite aplicar directamente el JSON Schema completo porque contiene `allOf`; estas comprobaciones no validan el schema completo ni activan el runtime web.
